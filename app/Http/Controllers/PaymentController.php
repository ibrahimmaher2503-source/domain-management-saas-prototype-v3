...
final class PaymentController extends Controller
{
    ...
    {
        $payload = $request->all();
        if (! $verifier->verify($payload, $request->query('hmac') ?: $request->input('hmac') ?: data_get($payload, 'obj.hmac'))) {
            Log::warning('Paymob callback processed', ['outcome' => 'invalid_hmac']);
            abort(403);
        }
        if (($payload['type'] ?? '') !== 'TRANSACTION' || ! is_array($payload['obj'] ?? null)) {
            Log::info('Paymob callback processed', ['outcome' => 'ignored']);
        ...
        $outcome = DB::transaction(function () use ($providerOrderId, $transactionId, $transaction): string {
            ...
            if (($transaction['success'] ?? false) === true && ($transaction['pending'] ?? true) === false && $amountMatches && $currencyMatches) {
                ...
                if ($payment->order()->where('status', 'awaiting_payment')->update(['status' => 'paid']) === 1) {
                    DispatchPaidOrderFulfillment::dispatch($payment->order_id)->afterCommit();
                }
        ...
        Log::info('Paymob callback processed', ['outcome' => $outcome, 'provider_order_id' => $providerOrderId]);
