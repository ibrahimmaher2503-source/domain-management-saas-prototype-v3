...
export default function AdminLayout({ title, children }: PropsWithChildren<{ title: string }>) {
    ...
    return <><Head title={`${title} · Admin`} /><div className="min-h-screen bg-[#f7f7f7] text-[#171717]"><aside className="fixed inset-y-0 left-0 hidden w-60 bg-[#111] p-4 lg:block"><Link href={route('admin.overview')} className="mb-8 block px-3 text-lg font-semibold text-white">Domain Desk <span className="text-xs text-neutral-500">Admin</span></Link><Navigation /><Link href={route('overview')} className="absolute bottom-5 left-7 text-xs text-neutral-500 hover:text-white">Back to customer app</Link></aside><div className="lg:pl-60"><header className="flex h-16 items-center justify-between border-b bg-white px-4 sm:px-6"><button className="lg:hidden" aria-label="Toggle admin navigation" onClick={() => setOpen(!open)}>{open ? <X /> : <Menu />}</button><div><h1 className="font-semibold">{title}</h1><p className="text-xs text-neutral-500">Operations console</p></div><span className="text-sm text-neutral-500">{user.name}</span></header>{open && <div className="bg-[#111] p-4 lg:hidden"><Navigation /></div>}<main className="mx-auto max-w-[1440px] p-4 sm:p-6">{children}</main></div></div></>;
}
