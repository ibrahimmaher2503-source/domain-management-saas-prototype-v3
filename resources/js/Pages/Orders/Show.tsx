...
type Order = { id: number; domain: string; status: string; registration_period: number; customer_price: string; currency: string; nameservers: string[]; payment_error?: string; provisioning_failure_reason?: string; domain_id?: number; requires_reconciliation?: boolean };
...
export default function Show({ order }: { order: Order }) {
    ...
    const copy = order.requires_reconciliation ? ['Confirming registration', "We're confirming the registration status with the registrar."] : {
